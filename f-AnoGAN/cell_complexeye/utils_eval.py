...
import numpy as np
import scipy
from sklearn.metrics import confusion_matrix, roc_curve, accuracy_score, precision_recall_fscore_support, auc, \
...
def _test_step(self, final_volume, data_orig, data_seg, data_mask, batch_idx, ID, label_vol, ved_num):
    ...
    if self.cfg['medianFiltering']:
        diff_volume = torch.from_numpy(apply_3d_median_filter(diff_volume.numpy().squeeze(0).squeeze(0),
                                                              kernelsize=self.cfg.get('kernelsize_median',
                                                                                      5))).unsqueeze(
            0)  # bring back to tensor

    ### Compute Metrics per Volume / Step ###
    if self.cfg.evalSeg and self.dataset[0] not in self.healthy_sets and np.array(data_seg.squeeze().flatten()).astype(
            bool).any():  # only compute metrics if segmentation is available
        # Pixel-Wise Segmentation Error Metrics based on Differenceimage
        AUC, _fpr, _tpr, _threshs = compute_roc(diff_volume.squeeze(0).squeeze(0).flatten(),
                                                np.array(data_seg.squeeze(0).squeeze(0).flatten()).astype(bool))
        AUPRC, _precisions, _recalls, _threshs = compute_prc(diff_volume.squeeze(0).squeeze(0).flatten(),
                                                             np.array(data_seg.squeeze(0).squeeze(0).flatten()).astype(
                                                                 bool))

        # gready search for threshold
        bestDice, bestThresh = find_best_val(np.array(diff_volume.squeeze(0).squeeze(0)).flatten(),
        ...
        if not 'node' in self.dataset[0].lower():  # no 3D data
            diffs_thresholded = filter_3d_connected_components(
                diffs_thresholded.squeeze(0))  # this is only done for patient-wise evaluation atm
        ...
        diceScore = dice(np.array(diffs_thresholded.squeeze(0).squeeze(0)),
                         np.array(data_seg.squeeze(0).squeeze(0).flatten()).astype(bool))
        ...
        Haus = monai.metrics.compute_hausdorff_distance(diffs_thresholded.unsqueeze(0).unsqueeze(0), data_seg,
        ...
        for slice in range(data_seg.squeeze().shape[0]):
            ...
            if np.array(data_seg.squeeze()[slice].flatten()).astype(bool).any():
                self.eval_dict['DiceScorePerSlice'].append(dice(np.array(diff_volume.squeeze()[slice] > bestThresh),
                                                                np.array(data_seg.squeeze()[slice].flatten()).astype(
                                                                    bool)))
                PrecRecF1PerSlice = precision_recall_fscore_support(
                    np.array(data_seg.squeeze()[slice].flatten()).astype(bool),
                    np.array(diff_volume.squeeze()[slice] > bestThresh).flatten(), warn_for=tuple(), labels=[0, 1])
...
def _test_end(self):
    ...
    self.eval_dict['HausPerVolMean'] = np.nanmean(
        np.array(self.eval_dict['HausPerVol'])[np.isfinite(self.eval_dict['HausPerVol'])])
    self.eval_dict['HausPerVolStd'] = np.nanstd(
        np.array(self.eval_dict['HausPerVol'])[np.isfinite(self.eval_dict['HausPerVol'])])
    ...
    if 'test' in self.stage:
        ...
        AUC_mean, _fpr, _tpr, _threshs = compute_roc(np.array(self.eval_dict['AnomalyScoreMeanPerVol']),
                                                     np.array(self.eval_dict['labelPerVol']))
        AUPRC_mean, _precisions, _recalls, _threshs = compute_prc(np.array(self.eval_dict['AnomalyScoreMeanPerVol']),
                                                                  np.array(self.eval_dict['labelPerVol']))
        pred_mean = np.array(self.eval_dict['AnomalyScoreMeanPerVol']) > self.threshold['total_class_mean']
        self.eval_dict['ACCClassificationMeanPerVol'] = accuracy_score(np.array(self.eval_dict['labelPerVol']),
                                                                       pred_mean)
        self.eval_dict['AUCClassificationMeanPerVol'] = AUC_mean
        self.eval_dict['AUPRCClassificationMeanPerVol'] = AUPRC_mean
        AUC_max, _fpr, _tpr, _threshs = compute_roc(np.array(self.eval_dict['AnomalyScoreMaxPerVol']),
                                                    np.array(self.eval_dict['labelPerVol']))
        AUPRC_max, _precisions, _recalls, _threshs = compute_prc(np.array(self.eval_dict['AnomalyScoreMaxPerVol']),
                                                                 np.array(self.eval_dict['labelPerVol']))
        ...
        AUC_patch, _fpr, _tpr, _threshs = compute_roc(np.array(self.eval_dict['AnomalyScorePatchPerVol']),
                                                      np.array(self.eval_dict['labelPerVol']))
        AUPRC_patch, _precisions, _recalls, _threshs = compute_prc(np.array(self.eval_dict['AnomalyScorePatchPerVol']),
                                                                   np.array(self.eval_dict['labelPerVol']))
        pred_patch = np.array(self.eval_dict['AnomalyScorePatchPerVol']) > self.threshold['total_class_patch']
        self.eval_dict['ACCClassificationPatchPerVol'] = accuracy_score(np.array(self.eval_dict['labelPerVol']),
                                                                        pred_patch)
    ...
    if 'val' in self.stage:
        if self.dataset[0] not in self.healthy_sets:
            bestdiceScore, bestThresh = find_best_val((self.diffs_list).flatten(),
            ...
            bestaccmean, bestmeanThresh = find_best_class_threshold(np.array(self.eval_dict['AnomalyScoreMeanPerVol']),
                                                                    np.array(self.eval_dict['labelPerVol']),
                                                                    val_range=(0, np.max(
                                                                        (self.eval_dict['AnomalyScoreMeanPerVol']))),
                                                                    max_steps=10,
                                                                    step=0,
                                                                    max_acc=0,
                                                                    best_threshold=0)
            self.threshold['total_class_mean'] = bestmeanThresh
            bestaccmax, bestmaxThresh = find_best_class_threshold(np.array(self.eval_dict['AnomalyScoreMaxPerVol']),
                                                                  np.array(self.eval_dict['labelPerVol']),
                                                                  val_range=(
                                                                  0, np.max((self.eval_dict['AnomalyScoreMaxPerVol']))),
                                                                  max_steps=10,
                                                                  step=0,
                                                                  max_acc=0,
                                                                  best_threshold=0)
            self.threshold['total_class_max'] = bestmaxThresh
            bestaccpatch, bestpatchThresh = find_best_class_threshold(
                np.array(self.eval_dict['AnomalyScorePatchPerVol']), np.array(self.eval_dict['labelPerVol']),
            ...
            if self.cfg.get('KLDBackprop', False):
                bestdiceScoreKLComb, bestThreshKLComb = find_best_val((self.diffs_listKLComb).flatten(),
                                                                      (self.seg_list).flatten().astype(bool),
                ...
                bestdiceScoreKL, bestThreshKL = find_best_val((self.diffs_listKL).flatten(),
                                                              (self.seg_list).flatten().astype(bool),
            ...
            _, fpr_healthy, _, threshs = compute_roc((self.diffs_list).flatten(),
                                                     np.zeros_like(self.diffs_list).flatten().astype(int))
