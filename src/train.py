        ...
        for fold in range(0, 5):
            if cfg.get("test_after_training"): # and not 'simclr' in  cfg.model._target_.lower():
                ...
                for set in cfg.datamodule.cfg.testsets :
                    ...
                    model.fold = fold   # set fold in model for logging purposes
                ...
                if cfg.get('pickle_preds', True):
                    out_path = os.path.join(trainer.log_dir, f'{fold + 1}_preds_dict.txt')
                    with open(out_path, 'w') as f:
                        pprint.pprint(preds_dict, stream=f, width=160, compact=False)
