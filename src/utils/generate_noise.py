...
def gen_noise(cfg,shape):
    ...
    elif cfg.noisetype == 'coarse':
        ns = generate_coarse_noise(shape, cfg.noise_std, cfg.noise_res).half()
    else: 
        raise ValueError('Noise type not recognized')
    return ns


# Adpated from https://github.com/AntanasKascenas/DenoisingAE/blob/master/src/denoising.py
def generate_coarse_noise(shape, noise_std, noise_res):
    ns = torch.normal(mean=torch.zeros(shape[0], shape[1], noise_res, noise_res), std=noise_std)

    ns = F.upsample_bilinear(ns, size=[shape[2], shape[3]])

    # Roll to randomly translate the generated noise.
    roll_x = random.choice(range(shape[2]))
    roll_y = random.choice(range(shape[3]))
