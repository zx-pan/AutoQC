...
class GaussianDiffusion(nn.Module):
    ...
    def p_losses(self, x_start, t, cond = None, noise = None, box=None, scale_patch=1, onlybox=False, mask=None, ssim_window_size=5):
        ...
        if self.cfg.get('model_name', 'DDPM') == 'VAE':
            model_out, mu, logvar = self.model(x, t, cond = None)
        else:
        ...
        if self.loss_type == 'ssim':
            ssim = tgm.losses.SSIM(window_size=ssim_window_size, reduction='none')
            loss = ssim(model_out, target)
        elif self.loss_type == 'l1+ssim':
            ssim = tgm.losses.SSIM(window_size=ssim_window_size, reduction='none')
            loss = self.cfg.get('alpha', 0.5)  * ssim(model_out, target) + (1 - self.cfg.get('alpha', 0.5) ) * F.l1_loss(model_out, target, reduction='none')
            if self.cfg.get('model_name', 'DDPM') == 'VAE':
