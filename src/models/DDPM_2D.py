...
from src.models.modules.OpenAI_AE import AEModel as OpenAI_AE
from src.models.modules.OpenAI_AE import VAEModel as OpenAI_VAE
...
class DDPM_2D(LightningModule):
    def __init__(self,cfg,prefix=None):
        ...
        # Model
        if cfg.get('model_name', 'DDPM') == 'AE':
            model = OpenAI_AE(
                            image_size =  (int(cfg.imageDim[0]),int(cfg.imageDim[1])),
                            in_channels = 1,
        ...
        elif cfg.get('model_name', 'DDPM') == 'VAE':
            model = OpenAI_VAE(
                            image_size =  (int(cfg.imageDim[0]),int(cfg.imageDim[1])),
                            in_channels = 1,
                            model_channels = cfg.get('unet_dim',64),
        ...
        model,
        image_size = (int(cfg.imageDim[0]),int(cfg.imageDim[1])), # only important when sampling
    ...
    def training_step(self, batch, batch_idx: int):
        ...
        if self.cfg.get('noisetype') is not None:
            ...
            noise = None
        # reconstruct
        if self.cfg.get('fix_timestep', False):
            loss, reco = self.diffusion(input, t=self.test_timesteps - 1, noise=noise)
        else:
    ...
    def test_step(self, batch: Any, batch_idx: int):
        ...
        def generate_rec(input):
            ...
            # reconstruct
            self.loss_diff, reco = self.diffusion(input,t=self.test_timesteps-1,noise=noise)
            return reco

        roi_size = self.cfg.get('imageDim', [128, 128, 70])
        roi_size = (roi_size[0], roi_size[1])
        reco = sliding_window_inference(input, predictor=generate_rec, roi_size=roi_size, sw_batch_size=1, overlap=0.8, mode="gaussian")

        # calculate loss and Anomalyscores
        AnomalyScoreComb.append(self.loss_diff.cpu())
        AnomalyScoreReg.append(self.loss_diff.cpu()) # dummy
        AnomalyScoreReco.append(self.loss_diff.cpu()) # dummy
        ...
        # calculate metrics
        _test_step(self, final_volume, data_orig, data_seg, data_mask, batch_idx, ID, label, ved_num) # everything that is independent of the model choice
