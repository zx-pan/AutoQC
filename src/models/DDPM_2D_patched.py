...
class DDPM_2D(LightningModule):
    ...
    def training_step(self, batch, batch_idx: int):
        ...
        if self.cfg.get('fix_timestep', False):
            loss, reco = self.diffusion(input, t=self.test_timesteps - 1, box=bbox, noise=noise)
        else:
    ...
    def test_step(self, batch: Any, batch_idx: int):
        ...
        self.loss_diff = 0
        input = batch['vol'][tio.DATA].float()
        data_orig = batch['vol_orig'][tio.DATA].float()
        ...
        def generate_rec(input):
            ...
            for k in range(bbox.shape[1]):
                box = bbox[:,k]
                # reconstruct
                self.loss_diff, reco = self.diffusion(input,t=self.test_timesteps-1, box=box,noise=noise)
        ...
        # calculate metrics
        _test_step(self, final_volume, data_orig, data_seg, data_mask, batch_idx, ID, label, ved_num)
